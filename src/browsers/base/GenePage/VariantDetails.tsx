...
// @ts-expect-error: this version of @gnomad/ui has no types
...
const findMostSevereConsequence = (consequences: string[]) => {
...
interface RawTranscriptConsequence {
  gene_id: string
  gene_symbol: string
  transcript_id: string
  canonical: NumberBool
  mane_select: NumberBool
  hgvsp: string
  hgvsc: string
  hgnc_id: string
  domains: string
...
type NestedParsedTranscript = Omit<ParsedTranscript, 'geneId' | 'geneSymbol' | 'majorConsequence'>

type GroupedConsequenceMap = Record<
  string,
  {
    consequence: string
    genes: Record<
      string,
      {
        geneId: string
        geneSymbol: string
        // transcripts: Omit<ParsedTranscript, 'geneId' | 'geneSymbol' | 'majorConsequence'>[],
        // transcripts: ParsedTranscript[],
        transcripts: NestedParsedTranscript[]
      }
    >
  }
>

const parseTranscriptConsequences = (transcriptConsequences: RawTranscriptConsequence[]) => {
  const reshaped: ParsedTranscript[] = transcriptConsequences.map((tc) => ({
    geneId: tc.gene_id,
  ...
  const groupedByConsequence = reshaped.reduce((acc: GroupedConsequenceMap, transcript) => {
    ...
    const { geneId, geneSymbol, majorConsequence: _, ...transcriptData } = transcript
...
const TranscriptDetails = ({ transcript }: { transcript: NestedParsedTranscript }) => {
  return (
    <ListItem>
      <div>
        {
...
const TranscriptConsequences = ({
  transcriptConsequencesString,
}: {
  transcriptConsequencesString: string
}) => {
  const transcriptConsequences: RawTranscriptConsequence[] = JSON.parse(
    transcriptConsequencesString
  ).filter((tc: RawTranscriptConsequence) => tc.gene_id.startsWith('ENSG'))
...
const VariantDetails = ({
  ...
  const standardColumns: VariantColumnConfig[] = [
    { key: 'group_result.ac_case', heading: 'AC Case', render: (value: number) => value },
    { key: 'group_result.an_case', heading: 'AN Case', render: (value: number) => value },
    { key: 'group_result.ac_ctrl', heading: 'AC Ctrl', render: (value: number) => value },
    { key: 'group_result.an_ctrl', heading: 'AN Ctrl', render: (value: number) => value },
    {
      key: 'group_result.af_case',
      heading: 'AF Case',
      render: (value: number) => renderExponential(value),
    },
    {
  ...
  if (datasetId === 'GP2') {
    const parsedGroupResults = variantAnalysisGroupOptions.reduce(
      (acc: Record<string, any>, group) => {
