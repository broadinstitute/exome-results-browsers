...
const Button = styled.button`
  padding: 10px 20px;
  border: none;
  margin-left: ${(props: ButtonProps) => (props.variant === 'logout' ? '20px;' : '')};
  background-color: ${(props: ButtonProps) => (props.variant === 'logout' ? '#e6004c' : '#7b558c')};
  color: white;
  border-radius: 4px;
...
const LoginPage = () => {
  ...
  const handleLogin = (e: React.SubmitEvent<HTMLElement>) => {
    e.preventDefault()
    setError('')
    setLoading(true)
